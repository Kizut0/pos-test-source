...
export function filterSalesByPeriod(sales, period) {
    // period: "daily" (today), "weekly" (last 7 days), "monthly" (this month)
    const now = new Date();
    const todayISO = now.toISOString().slice(0, 10);

    if (period === "daily") {
        return sales.filter((s) => toDateOnlyISO(s.date) === todayISO);
    }
...
export function salesByProduct(sales) {
...
export function topSellingItems(sales, n = 5) {
    return salesByProduct(sales).slice(0, n);
}
